...
export function GoalCard({ goal, sheetId, onEdit }: { goal: Goal, sheetId: string, onEdit: (g: Goal) => void }) {
  ...
  return (
    <div className={`flex flex-col gap-4 p-5 bg-zinc-900/40 hover:bg-zinc-900/60 border ${isShared ? 'border-blue-500/30' : 'border-zinc-800'} rounded-xl transition-colors group relative overflow-hidden`}>
      {isShared && (
        ...
        <div className="space-y-1">
          {isShared && (
            <div className="flex items-center gap-2 mb-2">
              <span className="px-2 py-0.5 bg-blue-500/10 text-blue-400 border border-blue-500/20 text-[10px] uppercase tracking-wider font-semibold rounded flex items-center gap-1">
                <Users size={10} /> Shared KPI
              </span>
            </div>
          )}
