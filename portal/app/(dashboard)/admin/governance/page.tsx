...
export default async function AdminGovernancePage({ searchParams }: { searchParams: Promise<{ tab?: string; quarter?: string }> }) {
  ...
  const unlockRequestsCount = sheets.filter(s => s.status === "LOCKED" && s.unlockRequested).length;

  // If in audit tab, fetch logs
  let auditLogs: any[] = [];
  if (currentTab === "audit") {
        ...
        <div className="flex items-center gap-3">
          {activeCycle && (
            <div className="flex items-center gap-2 bg-zinc-950/60 px-3 py-1.5 border border-zinc-900 rounded-lg">
              ...
              <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Active: Q2 Execution &bull; Q3 Planning</span>
            </div>
          )}
        </div>
      </div>
      ...
      {currentTab === "overview" ? (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="p-4 bg-zinc-900/40 border border-zinc-900 rounded-xl flex items-center gap-4">
              <div className="p-2.5 bg-zinc-950 text-zinc-400 border border-zinc-900 rounded-lg"><Users size={20} /></div>
              <div>
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Total Participants</p>
                <p className="text-xl font-extrabold text-zinc-100 font-mono mt-0.5">{totalEmployees}</p>
              </div>
            </div>
            
            <div className="p-4 bg-zinc-900/40 border border-zinc-900 rounded-xl flex items-center gap-4">
              <div className="p-2.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-lg"><Lock size={20} /></div>
              <div>
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Finalized / Active</p>
                <p className="text-xl font-extrabold text-emerald-450 font-mono mt-0.5">{lockedCount}</p>
              </div>
            </div>

            <div className="p-4 bg-zinc-900/40 border border-zinc-900 rounded-xl flex items-center gap-4">
              <div className="p-2.5 bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded-lg"><FileText size={20} /></div>
              <div>
            ...
            <div className="p-4 bg-zinc-900/40 border border-zinc-900 rounded-xl flex items-center gap-4">
              <div className="p-2.5 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-lg"><Calendar size={20} /></div>
              <div>
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">In Planning Workspace</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-1">
                  <div>
                    <h2 className="text-xs font-bold text-zinc-450 uppercase tracking-wider flex items-center gap-2">
                      Quarter Plan Overrides
                      {unlockRequestsCount > 0 && (
                        <span className="px-2 py-0.5 text-[9px] font-extrabold bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-full uppercase tracking-wider animate-pulse">
                          ⚡ {unlockRequestsCount} Request Outstanding
                        </span>
                      )}
                    </h2>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex bg-zinc-950 p-0.5 rounded-lg border border-zinc-900 text-[10px]">
                      <Link 
                ...
                <div className="bg-zinc-950/20 border border-zinc-900 rounded-2xl overflow-hidden shadow-sm">
                  {sheets.filter(s => s.status === "LOCKED" || s.status === "APPROVED").length === 0 ? (
                    <div className="p-8 text-center text-zinc-550 text-xs italic">No locked or approved quarter plans found to manage.</div>
                  ) : (
                        ...
                        <div key={sheet.id} className="p-4 flex items-center justify-between hover:bg-zinc-900/10 transition-colors">
                          <div>
                            <div className="flex items-center gap-2.5">
                              ...
                              <span className="px-2 py-0.5 text-[9px] font-bold bg-zinc-900 border border-zinc-850 text-zinc-400 rounded uppercase tracking-wider">{sheet.quarter}</span>
                              {sheet.unlockRequested && (
                                <span className="px-2 py-0.5 bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[9px] font-bold uppercase rounded-full tracking-wider animate-pulse flex items-center gap-1">
                                  <Unlock size={8} /> SLA Urgency: 12h
                                </span>
                              )}
                            </div>
                            <p className="text-xs text-zinc-500 mt-0.5">{sheet.user.email}</p>
                            {sheet.unlockRequested && (
                              <div className="bg-zinc-950 p-2.5 border border-zinc-900 rounded-lg mt-2 text-xs leading-relaxed max-w-lg">
                                <span className="block text-[9px] font-bold text-amber-500 uppercase tracking-wider mb-0.5">Manager Justification</span>
                                <span className="text-zinc-400 italic">"{sheet.unlockReason}"</span>
                              </div>
                            )}
                          </div>
                          <div className="flex items-center gap-4 shrink-0">
                            <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[9px] font-bold rounded-full uppercase tracking-wider">Locked</span>
            ...
            {/* Right sidebar: Cycle management & Department Heatmap */}
            <div className="lg:col-span-1 space-y-6">
              <CycleManagement cycles={cycles} activeCycle={activeCycle} />

              {/* Department Completion Heatmap panel */}
              <div className="bg-zinc-900/40 border border-zinc-900 rounded-2xl p-5 space-y-4">
                <h3 className="text-xs font-bold text-zinc-450 uppercase tracking-wider flex items-center gap-1.5">
                  <CheckSquare size={14} className="text-zinc-500" /> Department Compliance
                </h3>
                <p className="text-[11px] text-zinc-500 leading-relaxed">
                  Real-time quarter tracking rates per operational department.
                </p>
                <div className="space-y-3 pt-1">
                  <div className="space-y-1">
                    <div className="flex justify-between text-[11px] font-semibold text-zinc-400">
                      <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-cyan-500"></span> Engineering</span>
                      <span className="font-mono">92% Compliance</span>
                    </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[11px] font-semibold text-zinc-400">
                      <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Finance</span>
                      <span className="font-mono">100% Compliance</span>
                    </div>
                    <div className="h-1.5 w-full bg-zinc-950 rounded-full overflow-hidden">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[11px] font-semibold text-zinc-400">
                      <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-amber-500"></span> Customer Success</span>
                      <span className="font-mono">70% Compliance</span>
                    </div>
                    <div className="h-1.5 w-full bg-zinc-950 rounded-full overflow-hidden">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[11px] font-semibold text-zinc-400">
                      <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-rose-500"></span> HR</span>
                      <span className="font-mono">60% Compliance</span>
                    </div>
                    <div className="h-1.5 w-full bg-zinc-950 rounded-full overflow-hidden">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[11px] text-zinc-550 mt-1 leading-relaxed">Chronological record of plan proposals, manager approvals, target overrides, and system unlocks.</p>
          </div>
          ...
          <div className="bg-zinc-900/40 border border-zinc-900 rounded-2xl overflow-hidden shadow-xl">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-zinc-950 text-xs">
                  {auditLogs.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-12 text-center text-zinc-500 font-medium italic">No logs recorded.</td>
                    </tr>
                  ) : (
                        ...
                        <tr key={audit.id} className="hover:bg-zinc-900/10 transition-colors">
                          ...
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-zinc-300 font-semibold text-[11px]">{text}</span>
                            </div>
                          </td>
            ...
            <div className="p-3 bg-zinc-950 border-t border-zinc-900 text-center text-[10px] font-semibold text-zinc-550">
